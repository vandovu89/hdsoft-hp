...
function hdsoft_enqueue_scripts()
{
    ...
    wp_localize_script('hdsoft-script', 'themeTranslations', array(
        'ja' => array(
            ...
            // Product: NexusBridge AI
            'nexusbridge_title' => 'NexusBridge AI – AI搭載プロジェクト管理プラットフォーム',
            'nexusbridge_tagline' => 'AIを活用して言語の壁を越え、要件分析（ギャップ分析）やリスク予測を行うオフショア開発特化型のSaaS。',
            'nexusbridge_feature_1' => '<strong>AIギャップ分析：</strong>LLMが要件定義書を自動分析し、抜け漏れや矛盾を検出。',
            'nexusbridge_feature_2' => '<strong>多言語自動化：</strong>タスクやコメントを日・英・越・韓の間で自動翻訳・同期。',
            'nexusbridge_feature_3' => '<strong>リスクレーダー：</strong>開発プロセス（PR、コミット等）から進捗や品質のリスクを予測・警告。',
            'nexusbridge_feature_4' => '<strong>マルチテナント管理：</strong>エンタープライズ向けのセキュアな設計で、BYOK（Bring Your Own Key）にも対応。',
        ...
        'vi' => array(
            ...
            // Product: NexusBridge AI
            'nexusbridge_title' => 'NexusBridge AI – Nền tảng quản lý dự án AI',
            'nexusbridge_tagline' => 'Nền tảng quản lý dự án phát triển phần mềm chuyên biệt cho các team offshore, tích hợp AI để giải quyết các vấn đề về bất đồng ngôn ngữ, quản lý yêu cầu (gap analysis) và phân tích rủi ro dự án.',
            'nexusbridge_feature_1' => '<strong>Gap Analysis AI:</strong> Tự động phân tích tài liệu yêu cầu (Requirements) để phát hiện lỗ hổng và điểm thiếu sót bằng LLM.',
            'nexusbridge_feature_2' => '<strong>Tự động hóa đa ngôn ngữ:</strong> Dịch thuật tự động và đồng bộ giữa tiếng Nhật, Anh, Việt, Hàn cho task và comments.',
            'nexusbridge_feature_3' => '<strong>Risk Radar:</strong> Dự đoán và cảnh báo rủi ro về tiến độ, chất lượng trong quá trình phát triển (Pull Requests, Commits).',
            'nexusbridge_feature_4' => '<strong>Quản lý Multi-tenant:</strong> Kiến trúc multi-tenant an toàn cho doanh nghiệp (Enterprise), hỗ trợ BYOK.',
